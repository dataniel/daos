...
### `write_prettier_xlsx()` -- write to Excel with sensible defaults
...
many lines for what should be a one-liner. `write_prettier_xlsx()` is the
...
write_prettier_xlsx(mtcars, "output.xlsx")
...
write_prettier_xlsx(list(Hoved = mtcars, iris), "output.xlsx")
```
...
```{r, eval = FALSE}
# Add one sheet
write_prettier_xlsx(append = list(Bilag = airquality), path = "output.xlsx")
...
write_prettier_xlsx(mtcars, "output.xlsx", as_table = TRUE)

# Exclude columns from the #,##0 format
write_prettier_xlsx(mtcars, "output.xlsx", skip_fmt = "hp")
```
