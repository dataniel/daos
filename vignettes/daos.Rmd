...
`read_files()` expands [glue syntax](https://glue.tidyverse.org/) in
path strings, aborts immediately if any file is missing, and reads all
files with automatic format detection. A single path returns the object
directly; multiple paths return a named list with a progress bar.
...
every dataset supplied. Invaluable before joins or before binding with `read_files(out = "bind")`.
...
### `screen_timeseries()` — Shiny time-series screening dashboard

`screen_timeseries()` launches an interactive Shiny app for reviewing time
