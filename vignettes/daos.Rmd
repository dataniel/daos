...
1. **Convert** the PDFs to text files with `accounts_pdf_to_txt()`.
2. **Format** the text files by hand: financial statements are inherently
   table-like, so the relevant lines are shaped into the simple
   space-delimited format described under `accounts_txt_to_xlsx()` below.
3. **Parse and combine** all text files into a single validated Excel file
   with `accounts_txt_to_xlsx()`.
