...
where console output must be clean. The name is deliberately unusual --
a common word like `quiet` is one masking conflict away from confusion.
...
something has changed upstream. The workflow thinking behind this -- and
how `view_types()` backs it up when files refuse to stack -- has its own
article: `vignette("read-files")`.
...
`writexl::write_xlsx()` is fast but bare, and `openxlsx2` can do all the
styling but requires building a workbook object, adding worksheets,
applying styles, and saving. `write_excel()` is the middle ground: a
single call with defaults that make the file presentable.
...
vector-in/vector-out, so they slot into a `mutate()`:
...
Note the deliberate asymmetry: `clean_cpr()` zero-pads nine-digit values
(birth days 01--09 mean a third of all CPR numbers legitimately start
with a zero, which Excel drops), while `clean_cvr()` never invents a
digit -- a seven-digit CVR cannot be told apart from a typo, and padding
it would let a malformed number slip past a later
...
stripped first, so mixed input comes out uniform and the function is
idempotent. Any aggregation level works:
