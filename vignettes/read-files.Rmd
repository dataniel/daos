...
block with one `read_files()` call, and about using `view_types()` when
the files turn out not to be as stackable as they looked.
...
result with `readr::type_convert()`, and points you to `view_types()`:
...
- An `NA` means the column does not exist in that dataset at all
  (`year` vs `turnover` above), typically a rename or a dropped column
  upstream.
...
the checkpoint pattern from `vignette("validation")`:
...
assigns each file as its own variable. That is closest to the
