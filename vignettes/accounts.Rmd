---
title: "From PDF accounts to a tidy dataset"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{From PDF accounts to a tidy dataset}
...
2. **Format** the text files by hand: financial statements are inherently
   table-like, so the relevant lines are shaped into a simple
   space-delimited format.
3. **Parse and combine** all text files into a single validated Excel
   file with `accounts_txt_to_xlsx()`.
...
`cvr_*` pipeline feeds directly into step 1
(see `vignette("cvr", package = "daos")`).
...
and year, in both the returned tibble and the Excel file. The `elementid`
